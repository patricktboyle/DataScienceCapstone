...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        fig = px.pie(values=site_counts.values,names=site_counts.index,title=f'Split of Success/Failure for {entered_site}')
        return fig
# TASK 4:
...
def get_scatter_chart(entered_site,entered_payload):
    spacex_df['Booster Version Category'] = spacex_df['Booster Version'].apply(lambda x: x.split()[1] if len(x.split()) > 1 else None).copy()
    filtered_df = spacex_df[spacex_df['Launch Site'] == entered_site]
    if entered_site == 'ALL':
